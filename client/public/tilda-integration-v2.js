        ...
        sortedDays.forEach((day, dayIndex) => {
            ...
            for (const [trackName, trackData] of Object.entries(tracksObj)) {
                ...
                trackData.sessions.forEach(session => {
                    ...
                    html += `<h5 class="fw-bold mb-3">${session.name || 'Сессия'}</h5>`;
                   
                    if (questions.length > 0) {
                        html += `<ul class="list-unstyled mb-3">`;
                        questions.forEach((q, idx) => {
                            html += `<li class="d-flex mb-2 lh-sm small" style="font-size: 0.95rem;">`;
                            html += `<span>${q.title} ${q.body ? `<br><span class="text-muted">${q.body}</span>` : ''}</span>`;
                            html += `</li>`;
                        });
                        html += `</ul>`;
                    }
                    ...
                    function renderPerson(p) {
                        ...
                        return `<div class="lh-sm mb-3" style="font-size: 0.95rem;"><span class="fw-bold text-dark" style="font-size: 1.05rem;">${name}${pos ? ',' : ''}</span><br><span class="text-muted pb-1">${pos}</span></div>`;
                    }

                    if (moderators.length > 0) {
                        html += `<div class="fw-light mb-2 text-muted" style="font-size: 0.8rem !important; text-transform: uppercase;">Модератор${moderators.length > 1 ? 'ы' : ''}</div>`;
                        moderators.forEach(m => html += renderPerson(m));
                    }
                    if (speakers.length > 0) {
                        html += `<div class="fw-light mb-2 mt-3 text-muted" style="font-size: 0.8rem !important; text-transform: uppercase;">Спикеры</div>`;
