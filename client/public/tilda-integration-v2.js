        ...
        sortedDays.forEach((day, dayIndex) => {
            ...
            for (const [hallName, trackGroup] of Object.entries(hallsObj)) {
                ...
                for (const [trackName, sessionsArray] of Object.entries(trackGroup)) {
                    ...
                    sessionsArray.forEach(session => {
                        ...
                        const safeTitle = session.title || 'Сессия';
                        
                        // Если нет описания - просто карточка. Если есть - показываем кусочек
                        let shortDesc = "";
                        if (session.description && session.description !== 'undefined') {
                            shortDesc = session.description.substring(0, 100) + (session.description.length > 100 ? '...' : '');
                        ...
                        html += `
                            <div class="col-12 col-md-6 col-xl-4">
                                <div class="bg-body-tertiary p-3 rounded-2 position-relative h-100 custom-session-card" style="transition: background 0.2s;">
                                    ...
                                    <div class="fw-bold small mb-2">${time}</div>
                                    <div class="small lh-sm" style="color: #212529; font-size: 0.9rem; font-weight: 500;">${safeTitle}</div>
                                    ${shortDesc ? `<div class="text-muted mt-2" style="font-size: 0.75rem;">${shortDesc}</div>` : ''}
                                </div>
                            </div>
                        `;
                    });
            ...
            for (const [trackName, trackData] of Object.entries(tracksObj)) {
                ...
                trackData.sessions.forEach(session => {
                    ...
                    const moderators = (session.speakers || []).filter(s => s.role === 'Организатор' || s.isModerator).map(s => s.speaker).filter(Boolean);
                    const speakers = (session.speakers || []).filter(s => s.role !== 'Организатор' && !s.isModerator).map(s => s.speaker).filter(Boolean);

                    html += `<div class="row to-scroll border-top pt-3 mb-4" id="${sAnchor}" style="border-color: ${colors.accent} !important;">`;
                    
                    html += `<div class="col-12 col-lg-auto text-lg-end text-nowrap" style="min-width: 140px;">`;
                    html += `<h6 class="fw-bold">${time}</h6>`;
                    html += `</div>`;

                    html += `<div class="col-12 col-lg">`;
                    html += `<h5 class="fw-bold mb-3">${session.title || 'Сессия'}</h5>`;
                   
                    if (session.description && session.description !== 'undefined') {
                        html += `<div class="mb-3 lh-sm small">${session.description}</div>`;
                    }
                    if (questions.length > 0) {
                        html += `<ul class="list-unstyled mb-3">`;
                        questions.forEach((q, idx) => {
                            html += `<li class="d-flex mb-2 lh-sm small">`;
                            html += `<span class="fw-bold me-2" style="color: ${colors.accent}">#${idx+1}</span>`;
                            html += `<span>${q.title} ${q.body ? `— <span class="text-muted">${q.body}</span>` : ''}</span>`;
                            html += `</li>`;
                        });
                        html += `</ul>`;
                    }
                    ...
                    // Блок людей: Без фото, только текст
                    html += `<div class="col-12 col-lg-4 mt-3 mt-lg-0">`;
                    function renderPerson(p) {
                        const name = `${p.firstName || ''} ${p.lastName || ''}`.trim();
                        const pos = [p.position, p.company].filter(Boolean).join(', ');
                        return `<div class="lh-sm mb-2 small"><span class="fw-bold text-dark">${name}${pos ? ',' : ''}</span> ${pos}</div>`;
                    }

                    if (moderators.length > 0) {
                        html += `<div class="fw-light fs-6 mb-2 text-muted" style="font-size: 0.85rem !important; text-transform: uppercase;">Модератор${moderators.length > 1 ? 'ы' : ''}</div>`;
                        moderators.forEach(m => html += renderPerson(m));
                    }
                    if (speakers.length > 0) {
                        html += `<div class="fw-light fs-6 mb-2 mt-3 text-muted" style="font-size: 0.85rem !important; text-transform: uppercase;">Спикеры</div>`;
