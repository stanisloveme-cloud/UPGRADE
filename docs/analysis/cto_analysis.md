...
- Сервер — **Cloud NL-50 (2 x 3.3 ГГц CPU, 4 ГБ RAM, 50 ГБ NVMe)**. При таких ресурсах стек PostgreSQL + Redis + NestJS + Frontend должен работать стабильно, однако регулярные OOM свидетельствуют о критических утечках памяти, отсутствии `mem_limit` в контейнерах или некорректной настройке Docker/swap.
...
**PRD-006** декларирует `Min RAM: 4 GB`. Фактический сервер в точности соответствует этим требованиям, однако OOM Kills всё равно происходят. Это указывает на необходимость профилирования потребления памяти и установки жёстких лимитов.
...
graph LR
    ...
    A1["Убрать credentials\nиз docs/ в .env"] --> A
    A2["Удалить противоречие\nPowerShell vs Git Bash\nв rules"] --> A
    A3["Решить OOM:\nswap + resource limits\nв docker-compose.prod.yml"] --> A

    B1["Синхронизировать PRD-001\nи Prisma schema"] --> B
    B2["Multi-stage Dockerfile\n(build + runtime stages)"] --> B
    B3["Отключить Redis\nдо появления реального\nиспользования"] --> B

    C1["Один источник правды\nдля инфраструктуры\n(PROJECT_STATUS → deprecated)"] --> C
    C2["Очистить antigravity_kb.md\nот несуществующих ссылок"] --> C
```
