...
Использовать обертку `SafeDrawerForm` (базируется на `<DrawerForm>` из Ant Design Pro) шириной 1000px, обеспечивающую глобальный перехват ошибок валидации и предотвращение "белых экранов" через `ErrorBoundary`.

* **Trigger:** Клик по карточке сессии в Сетке (PRD-003).  
* **Title:** Редактирование сессии: {session.name}.  
* **Footer:** Явные кнопки "Сохранить изменения" и "Отмена" (используют встроенный `form.submit()`).
...
3. **Статусы (Dropdown):**
   ...
   * Под полем статуса мелким шрифтом выводится дата изменения статуса и инициалы пользователя (UI сверстан в виде отдельной вертикальной колонки, чтобы избежать смещения).
